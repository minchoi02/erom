...
<input type="hidden" id="opt_total_sum_price" name="opt_total_sum_price" value="0" />	
  ...
  <div id="sub_visual" class="sub_visual1">
  		<div class="inner">
          <div class="navigation">홈 <span>전체제품</span> <span class="last">펫케어</span></div>
          <div class="sub-title">펫케어</div>
      </div>
	</div>
	<!-- // 서브비주얼 영역 -->
        ...
        <div class="product-list">
            <div class="inner clearfix">
                ...
                <c:when test="${orderby eq 'Date'}"><a href="javascript:;">최신순</a></c:when>                
                    ...
                    <ul class="dropdown">
				<li><a href="/p_list.do?store_category=<c:out value="${store_category}"/>&prod_category=<c:out value="${prod_category}"/>&orderby=Recom" <c:if test="${orderby eq 'Recom'}">class="active"</c:if> >추천순</a></li>
				<li><a href="/p_list.do?store_category=<c:out value="${store_category}"/>&prod_category=<c:out value="${prod_category}"/>&orderby=Review" <c:if test="${orderby eq 'Review'}">class="active"</c:if> >리뷰순</a></li>
				<li><a href="/p_list.do?store_category=<c:out value="${store_category}"/>&prod_category=<c:out value="${prod_category}"/>&orderby=Sale" <c:if test="${orderby eq 'Sale'}">class="active"</c:if> >판매순</a></li>
				<li><a href="/p_list.do?store_category=<c:out value="${store_category}"/>&prod_category=<c:out value="${prod_category}"/>&orderby=Date" <c:if test="${orderby eq 'Date'}">class="active"</c:if> >최신순</a></li>               
                       <!--  <li><a href="#;">신상품</a></li>
                        <li><a href="#;">상품명</a></li>
                        <li><a href="#;">낮은가격</a></li>
                        <li><a href="#;">높은가격</a></li> -->
...
<%--                         <li>
 ...
 --%>
				...
				function jsPaging(pageNo) {
					...
					var linkurl = "/p_list.do?pageIndex="+pageNo;
