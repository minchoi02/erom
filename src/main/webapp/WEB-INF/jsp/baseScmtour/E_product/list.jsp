...
<input type="hidden" id="opt_total_sum_price" name="opt_total_sum_price" value="0" />	
  ...
  <div id="sub_visual" class="sub_visual1">
  		<div class="inner">
          <div class="navigation">홈 <span>전체제품</span> <span class="last">기능별</span></div>
          <div class="sub-title">기능별</div>
      </div>
	</div>
	<!-- // 서브비주얼 영역 -->
    ...
    <div id="container">
        ...
        <div class="product-list">
            <div class="inner clearfix">
                <ul id="tab2">
                    <li class="on"><a href="#;">눈건강</a></li>
                    <li><a href="#;">장건강/배변활동</a></li>
                    <li><a href="#;">#헬스케어</a></li>
                    <li><a href="#;">#바디케어</a></li>
                </ul>
                <div class="prd-sort">
                ...
                <c:when test="${orderby eq 'Date'}"><a href="javascript:;">최신순</a></c:when>                
                    ...
                    <ul class="dropdown">
				<li><a href="/list.do?store_category=<c:out value="${store_category}"/>&prod_category=<c:out value="${prod_category}"/>&orderby=Recom" <c:if test="${orderby eq 'Recom'}">class="active"</c:if> >추천순</a></li>
				<li><a href="/list.do?store_category=<c:out value="${store_category}"/>&prod_category=<c:out value="${prod_category}"/>&orderby=Review" <c:if test="${orderby eq 'Review'}">class="active"</c:if> >리뷰순</a></li>
				<li><a href="/list.do?store_category=<c:out value="${store_category}"/>&prod_category=<c:out value="${prod_category}"/>&orderby=Sale" <c:if test="${orderby eq 'Sale'}">class="active"</c:if> >판매순</a></li>
				<li><a href="/list.do?store_category=<c:out value="${store_category}"/>&prod_category=<c:out value="${prod_category}"/>&orderby=Date" <c:if test="${orderby eq 'Date'}">class="active"</c:if> >최신순</a></li>               
                       <!--  <li><a href="#;">신상품</a></li>
                        <li><a href="#;">상품명</a></li>
                        <li><a href="#;">낮은가격</a></li>
                        <li><a href="#;">높은가격</a></li> -->
                ...
                <div class="clearfix">
                	...
                	<c:forEach var="bestProdList" items="${bestProdList}" varStatus="status">
                        <li>
                            ...
                            <img src="/cmm/fms/getImage.do?atchFileId=<c:out value="${bestProdList.prod_img_1}"/>&fileSn=0" onerror="this.src='/images/baseScmtour/_noImage250.png';" alt="이미지"></a>
                            <div class="total-product-cover">
                              <div class="total-product-icon">
                                  <span><a href="<c:if test="${not empty user_id }">javascript:prod_pick('${bestProdList.prod_id }')"</c:if>
                                  <c:if test="${empty user_id }">javascript:getLogin('list.do')"</c:if>
                                  <i class="xi-heart-o"></i></a></span>
                                  <span><a href="/detail_view.do?store_category=<c:out value="${bestProdList.store_category}"/>&prod_category=<c:out value="${bestProdList.prod_category}"/>&prod_id=<c:out value="${bestProdList.prod_id}"/>"><i class="xi-cart-o"></i></a></span>
                                  <span><a href="/detail_view.do?store_category=<c:out value="${bestProdList.store_category}"/>&prod_category=<c:out value="${bestProdList.prod_category}"/>&prod_id=<c:out value="${bestProdList.prod_id}"/>"><i class="xi-credit-card"></i></a></span>
                              </div>
                            </div>                            
                            </div>
                            <div class="txtArea">
                              <div class="new-best"><span class="new">NEW</span><span class="best">BEST</span></div>  
                              <h1><c:out value="${bestProdList.prod_nm}"/></h1>
                              <span><c:out value="${bestProdList.prod_comment}"/></span>
                              <p><fmt:formatNumber value="${bestProdList.prod_price}" type="number"/>원</p>
                            </div>
